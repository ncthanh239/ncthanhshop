...
class ProductController extends Controller
{
    ...
    public function editList($id)
	{
		...
		->addColumn('code',function($sizecolor){
			$code=\App\ProductValue::where('product_id',$sizecolor->product_id)->where('attribute_id',19)->first();
		...
		->addColumn('name',function($sizecolor){
			$product=\App\ProductValue::where('product_id',$sizecolor->product_id)->where('attribute_id',20)->first();
			if(isset($product)){
    ...
    public function upload(Request $request){
		...
		// $path=$request->file->storeAs('public/images','image_'.time().'.png');
		// $path = $request->file->storeAs('public/images', 'image_'.time().'.png');
